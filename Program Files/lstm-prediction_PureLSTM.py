...
#C:\Users\rcwyu\OneDrive - hkoffice365\Computer Science\NEA\Programs
...
class tanhScaler():
    ...
    def fit_transform(self, data):
        data = np.array(data)
        try:
            self.scale = [sum([np.array(data).tolist()[i][j] for i in range(len(data))])/len(data) for j in range(len(data[0]))]
        except IndexError:
            self.scale = [sum([np.array(data).tolist()[i] for i in range(len(data))])/len(data)]
        self.maxParams = len(self.scale)
        return self.transform(np.array(data))

    def transform(self, dataL):
        dataL = np.array(dataL)
        if len(dataL.shape) == 1:
            return np.array([math.tanh(dataL[i]/self.scale[0]) for i in range(len(dataL))])
        elif self.maxParams > dataL.shape[1]:
            maxInstance = dataL.shape[1]
        else:
            maxInstance = self.maxParams
        return np.array([[math.tanh(dataL[i][j]/self.scale[j]) for j in range(maxInstance)] for i in range(len(dataL))])
    
    def inverse_transform(self, pos, data):
        return  np.array([self.scale[pos] * math.atanh(i) for i in data])
...
#'GBP_HKD Historical Data.csv' 'Shrinked GBP_HKD Historical Data.csv' 'EUR_USD Historical Data.csv' '2015 Dec 1 to 2019 Nov 29 Shrinked GBP_HKD Historical Data.csv'
dataFile, scaler, _ = 'Shrinked GBP_HKD Historical Data.csv', MinMaxScaler(), wx.App(False)
epochs, step_slice, validation_proportion, propagation_batch_size, pred_size = 60, 50, 0.1, 8, 5
neurons_per_layer = [300,250,200,150,100,50]
activation, optimizer, loss = 'tanh', 'adam', 'mean_squared_error'
load_weights_point, checkPrediction = 'null.hdf5', False
...
img_name = 'LSTM_Architecture_NoDropout.jpg'
...
def DeScale(array, scaler=scaler, scaling_method=str(scaler).split('(')[0]):
    if scaling_method == 'MinMaxScaler':
        ...
        return scaler.inverse_transform(0, array)
...
def randomTest(model, train_pred, valLoss, Ypred, output_price, vp=validation_proportion, ss=step_slice):
    closeGraphs()
    plt.plot(DeScale(output_price), color='black')
    for i in range(ss, len(train_pred)+ss-len(train_pred[0])):
        plt.plot([j for j in range(i,i+len(train_pred[i-ss]))], train_pred[i-ss], color='green')
    plt.plot([j for j in range(i+ss+len(train_pred[0]),i+ss+len(Ypred)+len(train_pred[0]))], Ypred, color='red')
    plt.title('Period')
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    plt.show()  

def testGradient(Ytest, Ypred):
    Ytgrad = [Ytest[i+1] - Ytest[i] for i in range(len(Ytest)-1)]
    Ypgrad = [Ypred[i+1] - Ypred[i] for i in range(len(Ypred)-1)]
    compLis = []
    for i in range(len(Ypgrad)):
        if ((Ytgrad[i] > 0 and Ypgrad[i] > 0) or (Ytgrad[i] < 0 and Ypgrad[i] < 0)):
            compLis.append(True)
        else:
            compLis.append(False)
    return compLis

print ('Import Complete\nData Splitting')
gc.enable()
test_data_len = len(data)-step_slice-pred_size
...
X_train, Y_train, output_price, X_test, Y_test = [], [], [i[0] for i in data_training] + [i[0] for i in data_test[:step_slice:]], [data_test[:step_slice:]], data_test[step_slice::, 0]

for i in tqdm(range(step_slice, len(data_training)-pred_size+1), desc='Splitting Data'):
    X_train.append(data_training[i-step_slice:i])
    Y_train.append(data_training[i:i+pred_size, 0])

X_train, Y_train, X_test, Y_test = np.array(X_train), np.array(Y_train), np.array(X_test), np.array(Y_test)
print ('Data Spltting Complete\n')
#Step III: Neural Network Implementation
start_time = time.time()
LSTM_model = Sequential()
LSTM_model.add(LSTM(units = neurons_per_layer[0], activation = activation, return_sequences = True, input_shape=(step_slice, X_train.shape[2])))
LSTM_model.add(Dropout(0.25))
for i in range(1, neural_network_layers-1):
    LSTM_model.add(LSTM(units = neurons_per_layer[i], activation = activation, return_sequences = True))
    LSTM_model.add(Dropout(0.25))
LSTM_model.add(LSTM(units = neurons_per_layer[-1], activation = activation))
LSTM_model.add(Dropout(0.25))
LSTM_model.add(Dense(units = pred_size))
LSTM_model.compile(optimizer='rmsprop', loss=loss)
cls()

try:
    plot_model(LSTM_model, to_file=img_name,show_shapes=True,expand_nested=True,dpi=720)
    #ann_viz(LSTM_model, filename='Neural Network Model.jpg', title='LSTM Time-Series RNN Model')
...
if not checkPrediction:
    history = LSTM_model.fit(
        ...
        callbacks=[Timings,
                ...
                TQDMCallback(metric_format='{name}: {value:0.10f}'),
                EarlyStopping(monitor='val_loss', min_delta=5e-6, patience=300, restore_best_weights=False)])
training_time = str(time.time()-start_time)
start_time = time.time()
Y_pred = LSTM_model.predict(X_test)[0]
train_pred = []
print ('')
for i in tqdm(range(len(X_train)), desc='Generating Training Sequence...'):
    train_pred.append(DeScale(LSTM_model.predict(np.array([X_train[i]]))[0]))
#train_pred = [DeScale(LSTM_model.predict(np.array([i]))[0]) for i in X_train]
predict_time = str(time.time()-start_time)
testG = testGradient(Y_test, Y_pred)
Y_training, Y_realData, Y_prediction = DeScale(output_price), DeScale(Y_test), DeScale(Y_pred)
rmse = RMSE(Y_realData, Y_prediction)
gc.collect()
...
if len(Timings.logs) > 2:
    ...
    Picture(box_l1, image=img_name, grid=[0,1], width=int(screen_res[0]*0.2), height=int(screen_res[0]*0.2*Image.open(img_name).size[1]/Image.open(img_name).size[0]))
    ...
    Text(box_l3, text=artifical_padding+'\nPrediction Statistics', size=def_text_size+3, grid=[0,2], color=def_colour)
    Text(box_l3, text=model_summary, size=def_text_size, grid=[0,1], align='left', color=def_colour)
    Text(box_l3, text=artifical_padding+'Gradients Correct: '+str(len([1 for i in testG if i])), size=def_text_size, grid=[0,3], align='left', color=def_colour)
    Text(box_l3, text=artifical_padding+'Gradients Inorrect: '+str(len([1 for i in testG if not i])), size=def_text_size, grid=[0,4], align='left', color=def_colour)
    ...
    createButton(box_l4, def_text_size, def_colour, text='Show Fitting Pattern', command=randomTest, args=[LSTM_model, train_pred, history.history['loss'], Y_prediction, output_price], grid=[1,3])
    ...
    createButton(graphApp, def_text_size, def_colour, text='Show Fitting Pattern', command=randomTest, args=[LSTM_model, train_pred, Y_train, [0], Y_prediction, Y_realData], grid=[0,3])
