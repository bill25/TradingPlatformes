...
epochs, step_slice, validation_proportion, propagation_batch_size = 20, 50, 0.3, 32
neurons_per_layer = [50,60,120,120]
dropout_per_layer = [0.2 for i in range(len(neurons_per_layer))]
activation, optimizer, loss, load_weights_point = 'relu', 'adam', 'mean_squared_error', '----.hdf5'
...
def createLog(run_time, restore, scaler, neural_network_layers,test_data_len, step_slice, validation_proportion,
              neurons_per_layer, dropout_per_layer, activation, optimizer, loss, propagation_batch_size,
    ...
    try:
        ...
        with open(str('Model_Summary '+str(str(run_time).replace(':','.'))+'.txt'), 'a+') as summary_f:
            summary_f.write('Backend Framework: '+str(backend.backend()).title()+'\n'+
                            ...
                            'Dropout Layer Configuration: '+str(dropout_per_layer)+'\n'+
...
LSTM_model.add(Dropout(dropout_per_layer[0]))
for i in range(1, neural_network_layers-1, 1):
    LSTM_model.add(LSTM(
        units = neurons_per_layer[i],
        activation = activation,
        return_sequences = True))
    LSTM_model.add(Dropout(dropout_per_layer[i]))
LSTM_model.add(LSTM(
    units = neurons_per_layer[-1],
    activation = activation))
LSTM_model.add(Dropout(dropout_per_layer[-1]))
LSTM_model.add(Dense(units = step_slice))
LSTM_model.compile(optimizer=optimizer, loss=loss)
cls()

try:
    plot_model(LSTM_model, to_file='LSTM_Architecture.jpg')
...
history = LSTM_model.fit(
    ...
    callbacks=[Timings,
               History(),
               TerminateOnNaN(),
               ModelCheckpoint('{epoch:03d}-val_loss {val_loss:.5f}-loss {loss:.5f}.hdf5', monitor='loss', save_best_only=True, mode='min'),
...
if len(Timings.logs) > 2:
    ...
    Picture(box_l1, image='LSTM_Architecture.jpg', width=int(screen_res[1]*0.85*Image.open('LSTM_Architecture.jpg').size[0]/Image.open('LSTM_Architecture.jpg').size[1]), height=int(screen_res[1]*0.85), grid=[0,1])
    ...
    Text(box_l2, text=artifical_padding+'Dropout Layer Configuration: '+str(dropout_per_layer), size=def_text_size, grid=[0,14], align='left', color=def_colour)
    ...
    createButton(box_l4, def_text_size, def_colour, text='Save Configurations', command=createLog, args=[run_time, restore, scaler, neural_network_layers,test_data_len, step_slice, validation_proportion,neurons_per_layer, dropout_per_layer, activation, optimizer, loss, propagation_batch_size, Timings.logs, model_init_time, X_train, training_time, predict_time, rmse, moving_average], grid=[0,1])
