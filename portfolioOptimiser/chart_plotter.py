...
class chart_plotter:
    ...
    def plot_Towassets_return_points(self, data):
        ax = plt.gca()
        columns = [c for c in data.columns if c not in 'Date']
        titleassets=" Assetes return for "+columns[0]+" And "+columns[1]
        data.plot(use_index=True, y=columns,ax=ax, title=titleassets,style=['o','rx'])
        plt.show()
